...
static inline void ringbuffer_advance_tail(struct scap_device* dev)
{
	...
	dev->m_bufinfo->mode = 0;
	//ioctl(dev->m_fd, PPM_IOCTL_BLOCK);
    // huanxing ;
	//printf("%d",ioctl(dev->m_fd, PPM_IOCTL_BLOCK));
	// if (ioctl(dev->m_fd, PPM_IOCTL_BLOCK)){
	// 		ASSERT(false);
	// 		return SCAP_FAILURE;
	// 	}
}
...
static inline bool are_buffers_empty(struct scap_device_set *devset)
{
	...
	for(j = 0; j < devset->m_ndevs; j++)
	{
		...
		if (ioctl(devset->m_devs[j].m_fd, PPM_IOCTL_BLOCK)){
			snprintf(devset->m_lasterr, SCAP_LASTERR_SIZE, "scap_enable_block_mode failed");
			ASSERT(false);
			return SCAP_FAILURE;
		}
	}
...
static inline void ringbuffer_advance_to_evt(scap_device* dev, scap_evt *event)
{
	...
	mem_barrier();
	dev->m_bufinfo->tail -= (event)->len;
	dev->m_sn_next_event += (event)->len;
	//printf("%d and %p\n", dev->m_bufinfo->tail, dev->m_sn_next_event);
}
#endif
...
static inline int32_t ringbuffer_next(struct scap_device_set *devset, OUT scap_evt** pevent, OUT uint16_t* pcpuid)
{
	...
	*pcpuid = 65535;
	...
	for(j = 0; j < ndevs; j++)
	{
		...
		mem_barrier();
		if (dev->m_bufinfo->mode!=2){
			dev->m_sn_next_event = dev->m_buffer;
			continue;
		}else if(dev->m_bufinfo->tail==0){
			printf("the cpu is %d\n",j);
			ADVANCE_TAIL(dev);
			continue;
		}else{
