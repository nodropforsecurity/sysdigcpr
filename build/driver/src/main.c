...
static int record_event_consumer(struct ppm_consumer_t *consumer,
				 ...
				 tp_values tp_type);
static int record_event_all_consumers(enum ppm_event_type event_type,
                                       enum syscall_flags drop_flags,
                                       struct event_data_t *event_datap,
				       tp_values tp_type);
...
#define DEFAULT_BUFFER_BYTES_DIM 8 * 1024;
...
int wakeup_flag = 0;
DECLARE_WAIT_QUEUE_HEAD (wq_cpr);
static struct task_struct *tsk;
static struct ppm_consumer_t *consumer = NULL;
// cpr
// rbtree denifition
#define RBTREE_TEST_INFO_SHOW
#ifdef RBTREE_TEST_INFO_SHOW
#define RBTREE_TEST_INFO(fmt,...) \
do{\
...
//stack
struct my_event_stack{
    struct evt_3 my_event;
	struct list_head head;
	struct rb_node nodes;
};

// typedef struct{
// 	struct evt_1 my_type;
//     struct list_head list_node;
// }id_struct2;
...
static int insert_item_to_mymap(struct rb_root *tree_root, struct my_data *item) {
	...
	while(*inserted_point) {
		...
		if(tmp_item == NULL){
			RBTREE_TEST_INFO("tmp_item is NULL");
			continue;
		}
        //int result = strcmp(item->keystring, tmp_item->keystring);
		parent = *inserted_point;

		if(item->keystring< tmp_item->keystring )
			inserted_point = &((*inserted_point)->rb_left);
		else if(item->keystring> tmp_item->keystring)
			inserted_point = &((*inserted_point)->rb_right);
		else
			return 1;
	}
	if(IS_ERR_OR_NULL(tree_root)){
		RBTREE_TEST_INFO("tree_root is NULL");
		return 1;
	}
...
static int insert_event_to_mymap(struct rb_root *tree_root, struct my_event *item) {
	struct rb_node **inserted_point = &(tree_root->rb_node);
	struct rb_node *parent = NULL;

	while(*inserted_point) {
		struct my_event *tmp_item = rb_entry(*inserted_point, struct my_event, nodes);
		if(tmp_item == NULL){
			RBTREE_TEST_INFO("tmp_item is NULL");
			continue;
		}
		...
		if(item->my_evt.id < tmp_item->my_evt.id)
			inserted_point = &((*inserted_point)->rb_left);
		else if(item->my_evt.id > tmp_item->my_evt.id)
			inserted_point = &((*inserted_point)->rb_right);
		else
			return 1;
	}
...
static int insert_event_reverse_to_mymap(struct rb_root *tree_root, struct my_event *item) {
	struct rb_node **inserted_point = &(tree_root->rb_node);
	struct rb_node *parent = NULL;
	...
	if(item->t_s < tmp_item->t_s)
			inserted_point = &((*inserted_point)->rb_left);
		else if(item->t_s > tmp_item->t_s)
			inserted_point = &((*inserted_point)->rb_right);
		else
			return 1;
	}
...
static int insert_event_stack_to_mymap(struct rb_root *tree_root, struct my_event_stack *item) {
	struct rb_node **inserted_point = &(tree_root->rb_node);
	struct rb_node *parent = NULL;

	while(*inserted_point) {
		struct my_event_stack *tmp_item = rb_entry(*inserted_point, struct my_event_stack, nodes);
		if(tmp_item == NULL){
			RBTREE_TEST_INFO("tmp_item is NULL");
			continue;
		}
		...
		int v1 = tmp_item->my_event.v;
		int u1 = tmp_item->my_event.u;
		char *sys_call1 = tmp_item->my_event.syscall;
		int v2 = item->my_event.v;
		int u2 = item->my_event.u;
		char *sys_call2 = item->my_event.syscall;
		char num[20];
		char hello[20];
		sprintf(num,"%d,%d,%s",v2,u2,sys_call2);
		sprintf(hello,"%d,%d,%s",v1,u1,sys_call1);
		int result = strcmp(num, hello);
        //int result = strcmp(item->keystring, tmp_item->keystring);
		parent = *inserted_point;
...
static struct my_event_stack *get_event_stack(struct rb_root *root, int v2, int u2, char* sys_call2) {
	struct rb_node *node = root->rb_node;

	while(node){
		struct my_event_stack *tmp_data = rb_entry(node, struct my_event_stack, nodes);

		if(tmp_data == NULL){
			RBTREE_TEST_INFO("tmp_data is NULL.[node]%p", node);
			continue;
		}

		int v1 = tmp_data->my_event.v;
		int u1 = tmp_data->my_event.u;
		char *sys_call1 = tmp_data->my_event.syscall;
		char num[20];
		char hello[20];
		sprintf(num,"%d,%d,%s",v2,u2,sys_call2);
		sprintf(hello,"%d,%d,%s",v1,u1,sys_call1);
		...
		if(result<0)
			...
			//printk("get %d and %d and %s",v1,u1,sys_call1);
			return tmp_data;
		}
	}
...
static struct my_data *get_data(struct rb_root *root, int string) {
	struct rb_node *node = root->rb_node;
	...
	while(node){
        ...
        // int result;
        // result = strcmp(string, tmp_data->keystring);
		if(string < tmp_data->keystring)
			node = node->rb_left;
		else if(string > tmp_data->keystring)
			node = node->rb_right;
		else
			return tmp_data;
	}
...
static struct my_event *get_event(struct rb_root *root, int64_t id) {
	struct rb_node *node = root->rb_node;

	while(node){
		struct my_event *tmp_data = rb_entry(node, struct my_event, nodes);
		...
		if(id < tmp_data->my_evt.id)
			node = node->rb_left;
		else if(id > tmp_data->my_evt.id)
			node = node->rb_right;
		else
			return tmp_data;
	}
...
static int replace_event(struct rb_root *root,int u, int v, char* syscall, int64_t id,  int64_t t_s, int64_t t_e)	{
	struct my_event *replace_item = kzalloc(sizeof(struct my_event), GFP_KERNEL);
	if(replace_item != NULL){
		replace_item->my_evt.u = u;
		replace_item->my_evt.v = v;
		replace_item->my_evt.syscall = syscall;
		replace_item->my_evt.id = id;
		replace_item->t_e = t_e;
		replace_item->t_s = t_s;
	}
	struct my_event *data = get_event(root, id);
	if(data != NULL)
		rb_replace_node(&data->nodes, &replace_item->nodes, root);
	kzfree(data);
	data = NULL;
	return 0;
}

static struct my_event *get_event_reverse(struct rb_root *root, int64_t second) {
	struct rb_node *node = root->rb_node;
	...
	while(node){
		...
		if(second < tmp_data->t_s)
			node = node->rb_left;
		else if(second > tmp_data->t_s)
			node = node->rb_right;
		else
			return tmp_data;
	}
...
// static int64_t _parse_time(struct ppm_evt_hdr *hdr, char *buffer, void *__data)
// {
//     size_t i;
...
static struct evt_1 _parse(struct ppm_evt_hdr *hdr, char *buffer, void *__data)
{
    size_t i;
    const struct ppm_event_info *info;
    const struct ppm_param_info *param;
    ...
    //printk("%lu %u : %s(", hdr->ts, hdr->tid, info->name);
    ...
    for (i = 0; i < info->nparams; ++i) {
        ...
        case PT_UINT8:
        case PT_SIGTYPE:
            //printk("%u" ,*(uint8_t *)data);
            break;
        case PT_FLAGS16:
        case PT_UINT16:
        case PT_SYSCALLID:
            //printk("%u" , *(uint16_t *)data);
            break;
        ...
        case PT_GID:
        case PT_SIGSET:
            //printk("%u" , *(uint32_t *)data);
            break;
        
        case PT_RELTIME:
        case PT_ABSTIME:
        case PT_UINT64:
            //printk("%u" , *(uint64_t *)data);
            break;

        case PT_INT8:
            //printk("%d" , *(int8_t *)data);
            break;

        case PT_INT16:
            //printk("%d" , *(int16_t *)data);
            break;
        
        case PT_INT32:
            //printk("%d" , *(int32_t *)data);
            break;
        ...
        case PT_FD:
        case PT_PID:
            //printk("%d" , *(int64_t *)data);
			all_data.v = *(int64_t *)data;
            break;
...
static void rb_release_data(struct my_map *gs_map){
...
static void rb_release_event(struct my_map *gs_map){
	struct rb_root *root = &gs_map->my_root;
	struct rb_node *node = rb_first(root);
	struct my_event *data;

	int i=0;
	//1.release every item
	while(node){
		data = rb_entry(node, struct my_event, nodes);
		//RBTREE_TEST_INFO("free the following item --->");
		//show_one_item(data);
...
static void rb_release_event_stack(struct my_map *gs_map){
	struct rb_root *root = &gs_map->my_root;
	struct rb_node *node = rb_first(root);
	struct my_event_stack *data;

	//1.release every item
	int i=0;
	while(node){
		data = rb_entry(node, struct my_event_stack, nodes);
		//RBTREE_TEST_INFO("free the following item --->");
		//show_one_item(data);
...
static bool check_overlap(int64_t start_1, int64_t end_1, int64_t start_2, int64_t end_2){
	 if (start_1 > start_2){
		int64_t temp = start_2;
        start_2 = start_1;
        start_1 = temp;
    ...
    if (start_2 <= end_1 || end_2 <= end_1)
        return true;

    return false;
}

static bool forward_check(int64_t id1, int64_t id2, int v, struct my_map *children, struct my_map *events){
	struct my_event *e1 = get_event(&events->my_root, id2);
	struct my_event *e2 = get_event(&events->my_root, id1);
	int64_t lower_limit = e1->t_s;
	int64_t upper_limit = e2->t_s;

	if(e1->t_s>e2->t_s){
		lower_limit = e2->t_s;
		upper_limit = e1->t_s;
	}
	struct my_data *data = get_data(&children->my_root, v);
	if (data == NULL){
		return true;
	}
	struct id_struct* ptr=NULL;
	list_for_each_entry(ptr, &data->head, list_node){
		int64_t id = ptr->my_type.id;
		struct my_event *e3 = get_event(&events->my_root, id);
		int64_t t1 = e3->t_s;
		int64_t t2 = e3->t_e;
		//printk("forward t1 is %lld and t2 is %lld",t1,t2);
		if (check_overlap(lower_limit,upper_limit,t1,t2)){
			//printk("hhhp");
			return false;
		}
    } 
...
static bool backward_check(int64_t id1, int64_t id2, int u, struct my_map *parents, struct my_map *events){
	struct my_event *e1 = get_event(&events->my_root, id2);
	struct my_event *e2 = get_event(&events->my_root, id1);
	int64_t lower_limit = e1->t_e;
	int64_t upper_limit = e2->t_e;

	if(e1->t_e>e2->t_e){
		lower_limit = e2->t_e;
		upper_limit = e1->t_e;
	}
	struct my_data *data = get_data(&parents->my_root, u);
	if (data == NULL){
		return true;
	}
	struct id_struct* ptr=NULL;
	list_for_each_entry(ptr, &data->head, list_node){
		...
		int64_t t1 = e3->t_s;
		int64_t t2 = e3->t_e;
		//printk("backward t1 is %lld and t2 is %lld",t1,t2);
		if (check_overlap(lower_limit,upper_limit,t1,t2)){
			//printk("hhh");
			return false;
		}
...
static int index(struct my_map *id,int v, int64_t id_){
	struct my_data *data = get_data(&id->my_root, v);
	struct id_num* ptr=NULL;
	int i =0;
	list_for_each_entry(ptr, &data->head, list_node){
		if (ptr->num == id_){
			return i; 
...
static void del_parents(struct my_map *id,int v, int id_){
	struct my_data *data = get_data(&id->my_root, v);
	struct id_struct* ptr=NULL;
	int i =0;
	//id_num * del = NULL;
	list_for_each_entry(ptr, &data->head, list_node){
		if (i == id_){
			__list_del_entry(&ptr->list_node);
			// kzfree(data);
			//data = NULL;
			return;
		}
...
static void del_parents_id(struct my_map *id,int v, int id_){
	struct my_data *data = get_data(&id->my_root, v);
	struct id_num* ptr=NULL;
	int i =0;
	//id_num * del = NULL;
...
static void del_events(struct my_map *id, int id_){
	struct my_event *data = get_event(&id->my_root, id_);
	rb_erase(&data->nodes, &id->my_root);
	return;
	// kzfree(data);
	// data = NULL;
}

// static unsigned long hash(struct evt_1 *sb, unsigned long hashval)
// {
//     unsigned long tmp;
...
static int cpr(void *data){
	do{
		wait_event_interruptible(wq_cpr, 
                                (test_bit(THREAD_WAKEUP, (unsigned long *)&wakeup_flag)
                                || kthread_should_stop()));
 
        clear_bit(THREAD_WAKEUP, (unsigned long *)&wakeup_flag);
        set_bit(THREAD_SLEEP, (unsigned long *)&wakeup_flag);
		/////////////////////////////////////////////////
		printk("starting cpr");
		ktime_t start, diff;
		start = ktime_get();
		struct ppm_ring_buffer_context * ring = (struct ppm_ring_buffer_context *)data;
		char *ptr, *buffer_end, *ptr_forward,*ptr_backward,*ptr_reserve,*buffer_end_forward,*buffer_end_backward,*buffer_end_reserve;
		struct ppm_evt_hdr *hdr1;
		struct ppm_evt_hdr *hdr2;
		ptr = ring->buffer;
		ptr_forward = ring->forward;
		ptr_backward = ring->backward;
		ptr_reserve = ring->reserve;
		buffer_end = ptr + ring->info->tail;
		// printk("%d",ring->info->tail);
		// printk("%p and %p",ptr,buffer_end);
		int count_forward=0;
		int count_backward=0;
		int count_reserve=0;
		while(ptr < buffer_end){
			hdr1 = (struct ppm_evt_hdr *)ptr;		
			if (hdr1->type==8 || hdr1->type==9 || hdr1->type==114 || hdr1->type==115 || hdr1->type==4 || hdr1->type==5){
				memcpy(ptr_backward,ptr,hdr1->len);
				ptr_backward += hdr1->len;
				count_backward++;
			}
			// else if(hdr1->type==6 || hdr1->type==7 || hdr1->type==116 || hdr1->type==117 || hdr1->type==102 || hdr1->type==103 || hdr1->type==306 || hdr1->type==307 || hdr1->type==154 || hdr1->type==155 || hdr1->type==12 || hdr1->type==13 || hdr1->type==292 || hdr1->type==293){
			else if(hdr1->type==6 || hdr1->type==7 || hdr1->type==116 || hdr1->type==117 || hdr1->type==102 || hdr1->type==103 || hdr1->type==306 || hdr1->type==307){
				memcpy(ptr_forward,ptr,hdr1->len);
				ptr_forward += hdr1->len;
				count_forward++;
			}
			else{
				memcpy(ptr_reserve,ptr,hdr1->len);
				ptr_reserve += hdr1->len;
				count_reserve++;
				//printk("type is %d",hdr1->type);
			}
			//_parse(hdr1, (char *)(hdr1 + 1), 0);
			ptr += hdr1->len;
		}
		buffer_end_forward = ptr_forward;
		ptr_forward = ring->forward;
		buffer_end_backward = ptr_backward;
		ptr_backward = ring->backward;
		buffer_end_reserve = ptr_reserve;
		ptr_reserve = ring->reserve;
		// while(ptr_forward<buffer_end_forward){
		// 	hdr1 = (struct ppm_evt_hdr *)ptr_forward;
		// 	_parse(hdr1, (char *)(hdr1 + 1), 0);
		// 	ptr_forward += hdr1->len;
		// }
	
		// while(ptr_reserve<buffer_end_reserve){
		// 	hdr1 = (struct ppm_evt_hdr *)ptr_reserve;
		// 	_parse(hdr1, (char *)(hdr1 + 1), 0);
		// 	ptr_reserve += hdr1->len;
		// }
		printk("cpr looping");
		...
		int u;
		char* sys_call;
		static struct my_map *parents;
		static struct my_map *children;
		static struct my_map *parents_id;
		static struct my_map *children_id;
		static struct my_map *events;
		static struct my_map *events_final;
		static struct my_map *events_reverse;
		static struct my_map *events_stack;
		parents_id = kzalloc(sizeof(struct my_map), GFP_KERNEL);
		if(IS_ERR_OR_NULL(parents_id)) {
			RBTREE_TEST_INFO("Allocating the memory for mMap is failed.");
			return -ENOMEM;
		}
		parents_id->name = "parents id";
		parents_id->my_root = RB_ROOT;// = (struct rb_root) { NULL, }

		children_id = kzalloc(sizeof(struct my_map), GFP_KERNEL);
		if(IS_ERR_OR_NULL(children_id)) {
			RBTREE_TEST_INFO("Allocating the memory for mMap is failed.");
			return -ENOMEM;
		}
		children_id->name = "children id";
		children_id->my_root = RB_ROOT;// = (struct rb_root) { NULL, }

		parents = kzalloc(sizeof(struct my_map), GFP_KERNEL);
		if(IS_ERR_OR_NULL(parents)) {
			RBTREE_TEST_INFO("Allocating the memory for mMap is failed.");
			return -ENOMEM;
		}
		parents->name = "parents";
		parents->my_root = RB_ROOT;// = (struct rb_root) { NULL, }

		children = kzalloc(sizeof(struct my_map), GFP_KERNEL);
		if(IS_ERR_OR_NULL(children)) {
			RBTREE_TEST_INFO("Allocating the memory for mMap is failed.");
			return -ENOMEM;
		}
		children->name = "children";
		children->my_root = RB_ROOT;// = (struct rb_root) { NULL, }


		events = kzalloc(sizeof(struct my_map), GFP_KERNEL);
		if(IS_ERR_OR_NULL(events)) {
			RBTREE_TEST_INFO("Allocating the memory for mMap is failed.");
			return -ENOMEM;
		}
		events->name = "events";
		events->my_root = RB_ROOT;// = (struct rb_root) { NULL, }

		events_final = kzalloc(sizeof(struct my_map), GFP_KERNEL);
		if(IS_ERR_OR_NULL(events_final)) {
			RBTREE_TEST_INFO("Allocating the memory for mMap is failed.");
			return -ENOMEM;
		}
		events_final->name = "events final";
		events_final->my_root = RB_ROOT;// = (struct rb_root) { NULL, }
		
		events_reverse = kzalloc(sizeof(struct my_map), GFP_KERNEL);
		if(IS_ERR_OR_NULL(events_reverse)) {
			RBTREE_TEST_INFO("Allocating the memory for mMap is failed.");
			return -ENOMEM;
		}
		events_reverse->name = "events reverse";
		events_reverse->my_root = RB_ROOT;// = (struct rb_root) { NULL, }

		events_stack = kzalloc(sizeof(struct my_map), GFP_KERNEL);
		if(IS_ERR_OR_NULL(events_stack)) {
			RBTREE_TEST_INFO("Allocating the memory for mMap is failed.");
			return -ENOMEM;
		}
		events_stack->name = "events stack";
		events_stack->my_root = RB_ROOT;// = (struct rb_root) { NULL, }

		int64_t id=0;


		while(ptr_forward<buffer_end_forward || ptr_backward<buffer_end_backward){
			struct evt_1 all_data1,all_data2;
			if(ptr_forward<buffer_end_forward){
				hdr1 = (struct ppm_evt_hdr *)ptr_forward;
				all_data1 = _parse(hdr1, (char *)(hdr1 + 1), 0);
				f_time_s = all_data1.id;
			}
			if(ptr_backward<buffer_end_backward){
				hdr2 = (struct ppm_evt_hdr *)ptr_backward;
				all_data2 = _parse(hdr2, (char *)(hdr2 + 1), 0);
				b_time_s = all_data2.id;
			}
			if ((ptr_backward==buffer_end_backward) || ((ptr_forward<buffer_end_forward) && (f_time_s<b_time_s))){
				v = all_data1.u;
				u = all_data1.v;
				sys_call = all_data1.syscall;
				t_start = f_time_s;
				ptr_forward += hdr1->len;
			}
			else{
				v = all_data2.v;
				u = all_data2.u;
				sys_call = all_data2.syscall;
				t_start = b_time_s;
				ptr_backward += hdr2->len;
			}

			struct id_num *a,*b;
			struct id_struct *c,*d;
			a = (struct id_num *)kzalloc(sizeof(struct id_num),GFP_KERNEL);
   			memset(a, 0, sizeof(struct id_num));
			b = (struct id_num *)kzalloc(sizeof(struct id_num),GFP_KERNEL);
			memset(b, 0, sizeof(struct id_num));
			c = (struct id_struct *)kzalloc(sizeof(struct id_struct),GFP_KERNEL);
   			memset(c, 0, sizeof(struct id_struct));
			d = (struct id_struct *)kzalloc(sizeof(struct id_struct),GFP_KERNEL);
   			memset(d, 0, sizeof(struct id_struct));


			if (get_data(&parents_id->my_root, v)==NULL){
				struct my_data *mydata_parents_id = kzalloc(sizeof(struct my_data),GFP_KERNEL);
				if(!IS_ERR_OR_NULL(mydata_parents_id)) {
				

					mydata_parents_id->keystring = v;
					INIT_LIST_HEAD(&mydata_parents_id->head);
					a->num = id;
					list_add(&a->list_node, &mydata_parents_id->head);
					if(insert_item_to_mymap(&parents_id->my_root, mydata_parents_id))
						RBTREE_TEST_INFO("insert the item %s to my map is failed", mydata_parents_id->keystring);
				}
				struct my_data *mydata_parents = kzalloc(sizeof(struct my_data),GFP_KERNEL);
				if(!IS_ERR_OR_NULL(mydata_parents)) {
					mydata_parents->keystring = v;
					INIT_LIST_HEAD(&mydata_parents->head);
					c->my_type.u = u;
					c->my_type.syscall = sys_call;
					c->my_type.id = id;
					list_add(&c->list_node, &mydata_parents->head);
					if(insert_item_to_mymap(&parents->my_root, mydata_parents))
						RBTREE_TEST_INFO("insert the item %s to my map is failed", mydata_parents->keystring);
				}
    		}else{
				a->num = id;
				c->my_type.u = u;
				c->my_type.syscall = sys_call;
				c->my_type.id = id;
				struct my_data *mydata_parents_id = get_data(&parents_id->my_root, v);
				struct my_data *mydata_parents = get_data(&parents->my_root, v);
				list_add_tail(&a->list_node,  &mydata_parents_id->head);
				list_add_tail(&c->list_node,  &mydata_parents->head);
			}
			
			if (get_data(&children_id->my_root, u)==NULL){
				struct my_data *mydata_children_id = kzalloc(sizeof(struct my_data),GFP_KERNEL);
				if(!IS_ERR_OR_NULL(mydata_children_id)) {
				
					mydata_children_id->keystring = u;
					INIT_LIST_HEAD(&mydata_children_id->head);
					b->num = id;
					list_add(&b->list_node, &mydata_children_id->head);
					if(insert_item_to_mymap(&children_id->my_root, mydata_children_id))
						RBTREE_TEST_INFO("insert the item %s to my map is failed", mydata_children_id->keystring);
				}
				struct my_data *mydata_children = kzalloc(sizeof(struct my_data),GFP_KERNEL);
				if(!IS_ERR_OR_NULL(mydata_children)) {
					mydata_children->keystring = u;
					INIT_LIST_HEAD(&mydata_children->head);
					d->my_type.u = v;
					d->my_type.syscall = sys_call;
					d->my_type.id = id;
					list_add(&d->list_node, &mydata_children->head);
					if(insert_item_to_mymap(&children->my_root, mydata_children))
						RBTREE_TEST_INFO("insert the item %s to my map is failed", mydata_children->keystring);
    			}
    		}else{
				b->num = id;
				d->my_type.u = v;
				d->my_type.syscall = sys_call;
				d->my_type.id = id;
				struct my_data *mydata_children_id = get_data(&children_id->my_root, u);
				struct my_data *mydata_children = get_data(&children->my_root, u);
				list_add_tail(&b->list_node,  &mydata_children_id->head);
				list_add_tail(&d->list_node,  &mydata_children->head);
			}

			struct my_event *mydata1 = kzalloc(sizeof(struct my_event),GFP_KERNEL);
			struct my_event *mydata2 = kzalloc(sizeof(struct my_event),GFP_KERNEL);
			struct my_event *mydata3 = kzalloc(sizeof(struct my_event),GFP_KERNEL);
			if(!IS_ERR_OR_NULL(mydata1)) {
				mydata1->my_evt.syscall = sys_call;
				mydata1->my_evt.u = u;
				mydata1->my_evt.v = v;
				mydata1->my_evt.id = id;
				mydata1->t_s = t_start;
				mydata1->t_e = t_start;
				if(insert_event_to_mymap(&events->my_root, mydata1))
					RBTREE_TEST_INFO("insert the item %d to my map is failed", mydata1->my_evt.id);
				// if(insert_event_to_mymap(&events_final->my_root, mydata1))
				// 	RBTREE_TEST_INFO("insert the item %d to my map is failed", mydata1->my_evt.id);
			}
			if(!IS_ERR_OR_NULL(mydata3)) {
				mydata3->my_evt.syscall = sys_call;
				mydata3->my_evt.u = u;
				mydata3->my_evt.v = v;
				mydata3->my_evt.id = id;
				mydata3->t_s = t_start;
				mydata3->t_e = t_start;
				if(insert_event_to_mymap(&events_final->my_root, mydata3))
					RBTREE_TEST_INFO("insert the item %d to my map is failed", mydata3->my_evt.id);
				// if(insert_event_to_mymap(&events_final->my_root, mydata1))
				// 	RBTREE_TEST_INFO("insert the item %d to my map is failed", mydata1->my_evt.id);
			}
			if(!IS_ERR_OR_NULL(mydata2)) {
				mydata2->my_evt.syscall = sys_call;
				mydata2->my_evt.u = u;
				mydata2->my_evt.v = v;
				mydata2->my_evt.id = id;
				mydata2->t_s = t_start;
				mydata2->t_e = t_start;
				if(insert_event_reverse_to_mymap(&events_reverse->my_root, mydata2))
					RBTREE_TEST_INFO("insert the item %d to my map is failed", mydata2->my_evt.id);
			}
			id++;
		}
		...
		struct rb_node *node; 
		struct my_map *map; 
		if(IS_ERR_OR_NULL(&events_reverse->my_root)) 
			RBTREE_TEST_INFO("the root is err."); 
		map = container_of(&events_reverse->my_root, struct my_map, my_root); 
		if(map == NULL) { 
			RBTREE_TEST_INFO("map is NULL"); 
			return -EINVAL; 
		} 
		int g =0;
		for(node = rb_first(&events_reverse->my_root); node; node = rb_next(node)){ 
			struct my_event *reverse = rb_entry(node, struct my_event, nodes);
			int v = reverse->my_evt.v;
			int u = reverse->my_evt.u;
			char* sys_call = reverse->my_evt.syscall;
			int64_t id = reverse->my_evt.id;
			//printk("event %d and %d and %s and %lld",v,u,sys_call,id);
			struct id_struct2* c2;
			struct id_struct2* del;

			c2 = (struct id_struct2 *)kzalloc(sizeof(struct id_struct2),GFP_KERNEL);
    		memset(c2, 0, sizeof(struct id_struct2));
			// printk("hhhhhhhhhhhhhhhhhhhh");
			// struct my_event_stack *stack = get_event_stack(&events_stack->my_root, v, u, sys_call);

			if (get_event_stack(&events_stack->my_root, v, u, sys_call)==NULL){
				struct my_event_stack *stack = kzalloc(sizeof(struct my_event_stack),GFP_KERNEL);
				if(!IS_ERR_OR_NULL(stack)) {
					//printk("ggg");
					stack->my_event.v = v;
					stack->my_event.u = u;
					stack->my_event.syscall = sys_call;
					INIT_LIST_HEAD(&stack->head);
					c2->my_type.u = u;
					c2->my_type.v = v;
					c2->my_type.syscall = sys_call;
					c2->my_type.id = id;
					list_add(&c2->list_node, &stack->head);
					if(insert_event_stack_to_mymap(&events_stack->my_root, stack))
						RBTREE_TEST_INFO("insert the item to my map is failed");
				}
    		}
			else{
				struct my_event_stack *stack = get_event_stack(&events_stack->my_root, v, u, sys_call);
				if (list_empty(&stack->head)){
					...
					//printk("ddd");
					del = list_entry((&stack->head)->prev,typeof(*del),list_node);
					int64_t id_candicate = del->my_type.id;
					int u_candicate = del->my_type.u;
					int v_candicate = del ->my_type.v;
					char* syscall_candicate = del ->my_type.syscall;
					__list_del_entry(&del->list_node);
					//printk("candicate %d and %d and %s and %lld",v_candicate,u_candicate,syscall_candicate,id_candicate);
					if ((forward_check(id_candicate, id, v, children, events)) && (backward_check(id_candicate, id, u, parents, events))){
						//printk("hhhhhhhhhhhhhhhhhhhhi");
						struct my_event *e1 = get_event(&events->my_root, id_candicate);
						struct my_event *e2 = get_event(&events->my_root, id);
						...
						//printk("hhhhhhhhhhhhhhhhhhhhp");
						replace_event(&events->my_root,u_candicate, v_candicate, syscall_candicate, id_candicate, lower_limit, upper_limit);
						// events_final[candidate_event] = (lower_limit, upper_limit,)
						//printk("hhhhhhhhhhhhhhhhhhhhq");
						replace_event(&events_final->my_root,u_candicate, v_candicate, syscall_candicate, id_candicate, lower_limit, upper_limit);
						//printk("hhhhhhhhhhhhhhhhhhhh5");
						int parents_index = index(parents_id, v, id);
						del_parents(parents,v,parents_index);
						del_parents_id(parents_id,v,parents_index);
						//printk("hhhhhhhhhhhhhhhhhhhh6");
						int children_index = index(children_id, u, id);
						del_parents(children,u,children_index);
						del_parents_id(children_id,u,children_index);
						//del events_final[event]
						//printk("hhhhhhhhhhhhhhhhhhhh7");
						del_events(events_final,id);
						if(!IS_ERR_OR_NULL(stack)) {
							c2->my_type.u = u_candicate;
							c2->my_type.v = v_candicate;
							c2->my_type.syscall = syscall_candicate;
							c2->my_type.id = id_candicate;
							list_add_tail(&c2->list_node,  &stack->head);
						}
						//printk("hhhhhhhhhhhhhhhhhhhh8");
					}
					else{
						// printk("hhhhhhhhhhhhhhhhhhhh9");
						if(!IS_ERR_OR_NULL(stack)) {
							c2->my_type.u = u;
							c2->my_type.v = v;
							c2->my_type.syscall = sys_call;
							c2->my_type.id = id;
							list_add_tail(&c2->list_node,  &stack->head);
							//struct id_struct2* ptr=NULL;
							// list_for_each_entry(ptr, &stack->head, list_node){
        					// 	printk("looping event g is%d and %d and %d and %s and %lld",g,ptr->my_type.v,ptr->my_type.u,ptr->my_type.syscall,ptr->my_type.id);
   							//  } 
							//printk("hhhhhhhhhhhhhhhhhhhh10");
						}
					}
				}
			}
		...
		// rb_release_event(events);
		// rb_release_event(events_reverse);
		rb_release_event(events_final);
		// rb_release_event_stack(events_stack);
		// rb_release_data(parents_id);
		// rb_release_data(parents);
		// rb_release_data(children_id);
		// rb_release_data(children);
		

		printk("%d and %d and %d",count_forward,count_backward,count_reserve);
...
static int ppm_open(struct inode *inode, struct file *filp)
{
	...
	if (!consumer) {
		...
		for_each_online_cpu(cpu) {
			...
			if(cpu==0){
				//tsk = kthread_run(cpr,NULL,"mythread%d",cpu);
				tsk = kthread_create(cpr,(void*)ring,"mythread%d",cpu);
				kthread_bind(tsk, 0);
				wake_up_process(tsk);
			}
		}
...
static int ppm_release(struct inode *inode, struct file *filp)
{
	...
	uint64_t total_evts = 0, total_drops = 0;
	for_each_possible_cpu(cpu) {
		ring = per_cpu_ptr(consumer->ring_buffers, cpu);
		total_evts += ring->info->n_evts;
		total_drops += ring->info->n_drops_buffer;
	}
	pr_info("%lu,%lu\n", total_evts, total_drops);
	...
	tp_values tp_type)
{
	...
	list_for_each_entry_rcu(consumer, &g_consumer_list, node) {
		res_block2 = record_event_consumer(consumer, event_type, drop_flags, ns, event_datap, tp_type);
	}
	rcu_read_unlock();
	return res_block2;
}
	...
	tp_values tp_type)
{
	...
	// }
	 ...
	 if (unlikely(ring->overflow.filled == 1 && event_type!=152)) {
        ring_info->tail = ((struct ppm_evt_hdr *)ring->overflow.addr)->len;
        //ring_info->n_evts++;
        memmove(ring->buffer, ring->overflow.addr, ring_info->tail);
        ring->overflow.filled = 0;
    }



	// head = ring_info->head;
	// ttail = ring_ring_info->tail;
	...
	freespace = consumer->buffer_bytes_dim - ring_info->tail;
	//printk("type %d, %d",event_type, freespace);
...
restart:
    if (freespace < args.arg_data_offset + sizeof(struct ppm_evt_hdr) || restart) {
        ...
        hdr = (struct ppm_evt_hdr *)ring->overflow.addr;
        args.buffer = ring->overflow.addr + sizeof(struct ppm_evt_hdr);
        args.buffer_size = PAGE_SIZE - sizeof(struct ppm_evt_hdr);
        ring->overflow.filled = 1;
		force = 1;
		//printk("small");
    } else {
        hdr = (struct ppm_evt_hdr *)(ring->buffer + ring_info->tail);
        args.buffer = ring->buffer + ring_info->tail + sizeof(struct ppm_evt_hdr);
        args.buffer_size = freespace - sizeof(struct ppm_evt_hdr);
    }

	if (!restart) {
        args.event_type = event_type;
        args.str_storage = ring->str_storage;
        args.nevents = ring_info->n_evts;
	...
	args.consumer = consumer;
		...
		if (likely(cbres == PPM_SUCCESS)) {
			...
			if (likely(args.curarg == args.nargs)) {
				...
				hdr->len = event_size;
				if (likely(ring->overflow.filled == 0)) {
                ring_info->tail += event_size;
                ++ring_info->n_evts;
            }
				drop = 0;
			} else {
...
// 	}
	...
	if (blocking == 0){
        ...
        flags = 0;
		set_bit(THREAD_WAKEUP, (unsigned long *)&wakeup_flag);
  		wake_up(&wq_cpr);
		return BLOCK;
	}	
...
TRACEPOINT_PROBE(syscall_enter_probe, struct pt_regs *regs, long id)
{
	...
	if (likely(table_index >= 0 && table_index < SYSCALL_TABLE_SIZE)) {
		...
		if (used)
			res_block =record_event_all_consumers(type, drop_flags, &event_data, SYS_ENTER);
		else
			res_block =record_event_all_consumers(PPME_GENERIC_E, UF_ALWAYS_DROP, &event_data, SYS_ENTER);
		if (res_block == BLOCK){
			if (!(strcmp(current->comm, "nginx") )){
				wait_event_interruptible(wq1, flags != 0);
			}
			if (!(strcmp(current->comm, "stress") )){
				wait_event_interruptible(wq2, flags != 0);
			}
...
TRACEPOINT_PROBE(syscall_exit_probe, struct pt_regs *regs, long ret)
{
	...
	if (used)
			res_block2 =record_event_all_consumers(type, drop_flags, &event_data, SYS_ENTER);
		else
			res_block2 =record_event_all_consumers(PPME_GENERIC_E, UF_ALWAYS_DROP, &event_data, SYS_ENTER);
        if (res_block2 == BLOCK){
			 if (!(strcmp(current->comm, "nginx") ) || !(strcmp(current->comm, "redis-server")) || !(strcmp(current->comm, "openssl"))){
			 	wait_event_interruptible(wq1, flags != 0);
			 }
			// if (!(strcmp(current->comm, "control") )){
			// 	wait_event_interruptible(wq2, flags != 0);
			// }
             else if (!(strcmp(current->comm, "stress") )){
				wait_event_interruptible(wq2, flags != 0);
			}
			
			// printk(KERN_INFO "name is %s",current->comm);
			// printk(KERN_INFO "flag is %d",flags);
...
TRACEPOINT_PROBE(syscall_procexit_probe, struct task_struct *p)
{
	...
	res_block3= record_event_all_consumers(PPME_PROCEXIT_1_E, UF_NEVER_DROP, &event_data, SCHED_PROC_EXIT);
}
...
#endif
{
	 ...
	 */
	res_block4 = record_event_all_consumers(PPME_SCHEDSWITCH_6_E, UF_USED | UF_ATOMIC, &event_data, SCHED_SWITCH);
}
#endif
...
static int init_ring_buffer(struct ppm_ring_buffer_context *ring, unsigned long buffer_bytes_dim)
{
	...
	//cpr
	ring->forward = vmalloc(buffer_bytes_dim + 2 * PAGE_SIZE);
	if (ring->forward == NULL) {
		pr_err("Error allocating ring memory\n");
		goto init_ring_err;
	}
	...
	ring->backward = vmalloc(buffer_bytes_dim + 2 * PAGE_SIZE);
	if (ring->backward == NULL) {
		pr_err("Error allocating ring memory\n");
		goto init_ring_err;
	}
	...
	ring->reserve = vmalloc(buffer_bytes_dim + 2 * PAGE_SIZE);
	if (ring->reserve == NULL) {
		pr_err("Error allocating ring memory\n");
		goto init_ring_err;
	}
	...
	ring->overflow.addr = (char *)__get_free_page(GFP_KERNEL);
    if (!ring->overflow.addr) {
...
static void free_ring_buffer(struct ppm_ring_buffer_context *ring)
{
    ...
    //cpr
	if (ring->forward) {
		vfree((void *)ring->forward);
		ring->forward = NULL;
	}

	if (ring->backward) {
		vfree((void *)ring->backward);
		ring->backward = NULL;
	}

	if (ring->reserve) {
		vfree((void *)ring->reserve);
		ring->reserve = NULL;
	}

	if (ring->overflow.addr) {
        free_page((unsigned long)ring->overflow.addr);
        ring->overflow.addr = 0;
        ring->overflow.filled = 0;
    }
